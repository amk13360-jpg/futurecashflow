...
export default function BuyerDetailsPage() {
 ...
 <CardContent>
 <div className="font-bold text-info text-2xl">{stats?.matched_invoices || 0}</div>
 </CardContent>
 </Card>
 <Card>
 <CardHeader className="pb-2">
 ...
 buyer.risk_tier === 'B' ? 'bg-warning-bg text-warning-foreground border border-warning-border' :
 ...
 log.approval_status === 'rejected' ? 'bg-error-bg text-error-foreground border border-error-border' :
 ...
 newDoc.file ? 'border-success-border bg-success-bg' : 'hover:border-primary'
 }`}
 ...
 {newDoc.file ? (
 <>
 <CheckCircle2 className="mx-auto mb-2 w-8 h-8 text-success" />
