...
export async function signStandingCession(
  ...
  try {
    ...
    // Generate login credentials on first cession sign (only if not already set)
    try {
      ...
      if (noCredentials.length > 0) {
        ...
        await query(
          ...
          [passwordHash, session.supplierId]
        )
        await sendSupplierCredentialsEmail(session.email, session.name, tempPassword)
        console.log(`[Standing Cession] Login credentials generated and emailed to ${session.email}`)
      }
    } catch (credError) {
      // Non-fatal — cession signing succeeded; credentials can be regenerated by admin if needed
      console.error("[Standing Cession] Warning: Could not generate supplier credentials:", credError)
    }
